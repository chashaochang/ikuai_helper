...
class NeuTextField extends StatefulWidget {
  ...
  /// [NeuTextField.noMaxLength] then only the current length is displayed.
  ///
  ...
  const NeuTextField({
    ...
    this.autofillHints,
        ...
        assert(maxLength == null || maxLength == NeuTextField.noMaxLength || maxLength > 0),
  ...
  /// to [NeuTextField.noMaxLength] then only the current character count is displayed.
  ///
  /// After [maxLength] characters have been input, additional input
  /// is ignored, unless [maxLengthEnforced] is set to false. The text field
  /// enforces the length with a [LengthLimitingTextInputFormatter], which is
  ...
  ///
  /// This value must be either null, [NeuTextField.noMaxLength], or greater than 0.
  /// If null (the default) then there is no limit to the number of characters
  /// that can be entered. If set to [NeuTextField.noMaxLength], then no limit will
...
class _NeuTextFieldState extends State<NeuTextField> implements TextSelectionGestureDetectorBuilderDelegate {
  TextEditingController _controller;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.decoration != null) {
      child = AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget child) => InputDecorator(
